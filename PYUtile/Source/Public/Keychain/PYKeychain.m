...
NSString *const kPYKeychainErrorDomain = @"com.samsoffes.PYKeychain";

NSString *const kPYKeychainAccountKey = @"acct";
NSString *const kPYKeychainCreatedAtKey = @"cdat";
NSString *const kPYKeychainClassKey = @"labl";
NSString *const kPYKeychainDescriptionKey = @"desc";
NSString *const kPYKeychainLabelKey = @"labl";
NSString *const kPYKeychainLastModifiedKey = @"mdat";
NSString *const kPYKeychainWhereKey = @"svce";

#if __IPHONE_4_0 && TARGET_OS_IPHONE  
CFTypeRef PYKeychainAccessibilityType = NULL;
#endif

@interface PYKeychain ()
+ (NSMutableDictionary *)_queryForService:(NSString *)service account:(NSString *)account;
@end

@implementation PYKeychain
...
+ (NSArray *)accountsForService:(NSString *)service error:(NSError **)error {
    OSStatus status = PYKeychainErrorBadArguments;
    ...
    if (status != noErr && error != NULL) {
		*error = [NSError errorWithDomain:kPYKeychainErrorDomain code:status userInfo:nil];
		return nil;
	}
...
#if __IPHONE_4_0 && TARGET_OS_IPHONE
		if (PYKeychainAccessibilityType) {
#if __has_feature(objc_arc)
...
+ (CFTypeRef)accessibilityType {
	return PYKeychainAccessibilityType;
}
...
+ (void)setAccessibilityType:(CFTypeRef)accessibilityType {
	...
	if (PYKeychainAccessibilityType) {
		CFRelease(PYKeychainAccessibilityType);
	}
	PYKeychainAccessibilityType = accessibilityType;
}
#endif
